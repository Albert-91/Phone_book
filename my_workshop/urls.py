...
urlpatterns = [
    ...
    url(r'^group-search/', GroupSearch.as_view(), name="search"),
    url(r'^person/(?P<id>[0-9]+)/', ShowDetail.as_view(), name="person"),
    url(r'^modify/(?P<id>\d+)$', ModifyPerson.as_view(), name="modify"),
    url(r'^Delete/(?P<data>\w+)/(?P<id>\d+)/(?P<id_person>\d+)$', DeleteData.as_view(), name="delete"),
    ...
    url(r'^(?P<id_person>\d+)/(?P<id_group>\d+)/EraseFromGroup/$', EraseFromGroup.as_view(), name="erase_from_group"),
]
